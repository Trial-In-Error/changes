...
int textFadeIn = 1000;
int textFadeOut = 1000;
int textWait = 1000;
int hexagramFadeIn = 1000;
int hexagramFadeOut = 1000;
int hexagramWait = 1000;
...
void fade(boolean brightening, int milliseconds) {
  if(brightening) {
    uView.contrast(0);
    for(int count = 0; count < 256; count++) {
      uView.contrast((int) (count));
      //Serial.print(String((int) ((count)))+" ");
      delay((int) milliseconds/255);
      uView.display();
    }
  } 
  else {
    uView.contrast(255);
    for(int count = 0; count < 256; count++) {
      uView.contrast((int) ((255 - count)));
      //Serial.print(String((int) (((255 - count))))+" ");
      delay((int) milliseconds/255);
      uView.display();
    }
  }
...
void drawHexagram(uint8_t hexagram[]) {
  ...
  //uView.display();
  fade(true, hexagramFadeIn);
  delay(hexagramWait);
  fade(false, hexagramFadeOut);
  delay(hexagramWait);
}
...
void drawText() {
  ...
  if(String(tokenPointer).length() > 10) {
    // if the text won't fit on one line
    // then print it without spaces inbetween characters
    for(uint8_t count = 0; count < String(tokenPointer).length(); count++) {
      uView.setCursor((uView.getLCDWidth() - uView.getFontWidth()*String(tokenPointer).length())/2 +uView.getFontWidth()*count, (uView.getLCDHeight() - uView.getFontHeight()*linesNeeded)/2);
      //uView.setCursor(uView.getFontWidth()*count, (uView.getLCDHeight() - uView.getFontHeight()*linesNeeded)/2);
      uView.print((char)tokenPointer[count]);
    }
  } 
  else {
    // print it normally
  ...
  while(tokenPointer != NULL) {
    ...
    if(tokenPointer != NULL) {
      if(String(tokenPointer).length() > 10) {
        ...
        // then print it without spaces inbetween characters
        for(uint8_t count = 0; count < String(tokenPointer).length(); count++) {
          uView.setCursor((uView.getLCDWidth() - uView.getFontWidth()*String(tokenPointer).length())/2 +uView.getFontWidth()*count, (uView.getLCDHeight() - uView.getFontHeight()*linesNeeded)/2 + lineCounter*uView.getFontHeight());
          //uView.setCursor(uView.getFontWidth()*count, (uView.getLCDHeight() - uView.getFontHeight()*linesNeeded)/2);
          uView.print((char)tokenPointer[count]);
        }
      } 
        ...
        // print it normally
        uView.setCursor((uView.getLCDWidth() - uView.getFontWidth()*String(tokenPointer).length() - String(tokenPointer).length() + 1)/2, (uView.getLCDHeight() - uView.getFontHeight()*linesNeeded)/2 + lineCounter*uView.getFontHeight());
        uView.print(String(tokenPointer));
      }
    }
  }
  ...
  fade(true, textFadeIn);
  delay(textWait);
  fade(false, textFadeOut);
  delay(textWait);
}
...
void serialReport() {
  ...
  Serial.println(String(textFadeIn)+", "+String(textFadeOut)+", "+String(textWait)+", "+String(hexagramFadeIn)+", "+String(hexagramFadeOut)+", "+String(hexagramWait));
}

void changeTimingsBySerial() {
  char * tokenPointer;
  while(Serial.available() > 0) {
    ...
    if (recieved == '\n') {
      char charBuf[30];
      inData.toCharArray(charBuf, 30);
      tokenPointer = strtok(charBuf, ",");
      textFadeIn =  String(tokenPointer).toInt();
      tokenPointer = strtok(NULL, ",");
      textFadeOut =  String(tokenPointer).toInt();
      tokenPointer = strtok(NULL, ",");
      textWait =  String(tokenPointer).toInt();
      tokenPointer = strtok(NULL, ",");
      hexagramFadeIn =  String(tokenPointer).toInt();
      tokenPointer = strtok(NULL, ",");
      hexagramFadeOut =  String(tokenPointer).toInt();
      tokenPointer = strtok(NULL, ",");
      hexagramWait = String(tokenPointer).toInt();
      Serial.println(String(textFadeIn)+", "+String(textFadeOut)+", "+String(textWait)+", "+String(hexagramFadeIn)+", "+String(hexagramFadeOut)+", "+String(hexagramWait));
      inData = "";
    }
  }
...
void changeHexagramBySerial() {
  while(Serial.available() > 0) {
    ...
    // Process message when new line character is recieved
    if (recieved == '\n') {
      globalCounter = inData.toInt();
      Serial.println("inData: "+String(inData.toInt()));
      if(!landscape) {
...
void loop () {
  ...
  //changeTimingsBySerial();
  //changeHexagramBySerial();
